...
class NavigationTransitioner extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    const nextScenes = NavigationScenesReducer(this.state.scenes, nextProps.navigationState, this.props.navigationState);

    if (nextScenes === this.state.scenes) {
      return;
    }

    const nextState = _extends({}, this.state, {
      scenes: nextScenes
    });
    ...
    const transitionSpec = _extends({}, DefaultTransitionSpec, transitionUserSpec);
    ...
    const animations = [timing(progress, _extends({}, transitionSpec, {
      toValue: 1
    }))];
  ...
  _onLayout(event) {
    ...
    const layout = _extends({}, this.state.layout, {
      initHeight: height,
      initWidth: width,
      isMeasured: true
    });
